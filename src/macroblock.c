...
void set_MB_parameters (int mb_addr)
{
  ...
  get_mb_block_pos(mb_addr, &img->mb_x, &img->mb_y);//以图像左上角为原点  得到当前宏块mb左上角的16*16坐标
  
  img->block_x = img->mb_x << 2;//以图像左上角为原点 得到当前宏块mb左上角的4*4坐标
  img->block_y = img->mb_y << 2;

  img->pix_x   = img->block_x << 2;//以图像左上角为原点 得到当前宏块mb左上角的像素坐标
  img->pix_y   = img->block_y << 2;

  img->opix_x   = img->pix_x;

  if (img->MbaffFrameFlag)//帧场自适应
  {
    if (img->mb_data[mb_addr].mb_field)//当前宏块是场模式
    {

      imgY_org  = (mb_addr % 2) ? imgY_org_bot  : imgY_org_top;//亮度 底场地址是奇数 顶场地址是偶数
      imgUV_org = (mb_addr % 2) ? imgUV_org_bot : imgUV_org_top;//色度 底场地址是奇数 顶场地址是偶数
      img->opix_y   = (img->mb_y >> 1 ) << 4;//不是很懂??
    }
    else
    {
      imgY_org  = imgY_org_frm;
      imgUV_org = imgUV_org_frm;
      img->opix_y   = img->block_y << 2;//转换成为像素坐标
    }
  }
  else //非帧场自适应
  {
  ...
  img->pix_c_x = img->pix_x >> 1;//色度的像素坐标 亮度减半
...
int clip1a(int a)
{
  return ((a)>255?255:((a)<0?0:(a)));//限幅在0到255
}
...
void proceed2nextMacroblock()
{
  ...
  if (p_trace)
  {
   ...
   // Write out the tracestring for each symbol
    for (i=0; i<currMB->currSEnr; i++)//对当前语法元素进行遍历
      trace2out(&(img->MB_SyntaxElements[i]));//将语法元素信息添加到trace文件里面
  }
#endif
...
void start_macroblock(int mb_addr, int mb_field)
{
  ...
  set_MB_parameters (mb_addr);//设置一些坐标变量等
  ...
  if(use_bitstream_backing)
  {
    ...
    if(!img->cod_counter)
      for (i=0; i<curr_slice->max_part_nr; i++)
      {
        ...
        if (input->symbol_mode ==CABAC)//使用CABAC进行熵编码
        {
          eep = &(dataPart->ee_cabac);
          eep->ElowS            = eep->Elow;//保存数据?
  ...
  if(input->RCEnable)
  {
    ...
    {    
      currMB->prev_qp = img->mb_data[img->current_mb_nr-1].qp;//前一个宏块的qp来预测当前宏块qp
