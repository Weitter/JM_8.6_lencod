...
void Configure(int ac, char *av[])
{
  ...
  memset (&configinput, 0, sizeof (InputParameters));//初始化0
  ...
  if (ac==2)
  {
    if (0 == strncmp (av[1], "-h", 2))//比较av[1]和"-h"前两个字节是否相同，则返回0
    {
      JMHelpExit();
    }
  }
 ...
 ***********************************************************************
 * \brief
 *    Checks the input parameters for consistency.一致性
 ***********************************************************************
 */
static void PatchInp ()
{
...
void PatchInputNoFrames() //Patch补丁
{
  // Tian Dong: May 31, 2002
...
static void ProfileCheck()
{
  ...
  {
    if (!input->directInferenceFlag)//Inference 推理
    {
