 ...
 ************************************************************************
 */
int mb_is_available(int mbAddr, int currMbAddr)//mbAddr待检测宏块地址，currMbAddr当前宏块地址
{
...
void CheckAvailabilityOfNeighbors()
{
  ...
  if (img->MbaffFrameFlag)
  {
    ...
    currMB->mbAvailA = mb_is_available(currMB->mbAddrA, mb_nr) && (((mb_nr/2) % img->PicWidthInMbs)!=0);//检测A块是否可用
  ...
  else
  {
	...
	/*
		 ...
		 A
	*/
    currMB->mbAvailA = mb_is_available(currMB->mbAddrA, mb_nr) && ((mb_nr % img->PicWidthInMbs)!=0);//左边缘
    currMB->mbAvailB = mb_is_available(currMB->mbAddrB, mb_nr);
    currMB->mbAvailC = mb_is_available(currMB->mbAddrC, mb_nr) && (((mb_nr+1) % img->PicWidthInMbs)!=0);//右上边缘
    currMB->mbAvailD = mb_is_available(currMB->mbAddrD, mb_nr) && ((mb_nr % img->PicWidthInMbs)!=0);//左上边缘
  }
 ...
 ************************************************************************
 */
void get_mb_block_pos (int mb_addr, int *x, int*y)//宏块左上角的宏块16*16坐标
{
  ...
  {
    *x = (mb_addr % img->PicWidthInMbs);//太聪明啦
 ...
 ************************************************************************
 */
void get_mb_pos (int mb_addr, int *x, int*y)//以图像左上角为原点 宏块左上角的像素坐标
{
...
/*
  ...
  */
  ...
  if ((xN<0)&&((yN>=0)&&(yN<maxWH)))//选中A宏块
  {
    pix->mb_addr  = currMb->mbAddrA;
    pix->available = currMb->mbAvailA;
  }
  else
  if (((xN>=0)&&(xN<maxWH))&&(yN<0))//选中B宏块
  {
    pix->mb_addr  = currMb->mbAddrB;
    pix->available = currMb->mbAvailB;
  }
  else
  if (((xN>=0)&&(xN<maxWH))&&((yN>=0)&&(yN<maxWH)))//选中当前宏块
  {
    pix->mb_addr  = curr_mb_nr;
    pix->available = 1;
  }
  else
  if ((xN>=maxWH)&&(yN<0))//选中C宏块
  {
  ...
  {
    pix->x = (xN + maxWH) % maxWH;//这不是就是等于xN？
    pix->y = (yN + maxWH) % maxWH;
    get_mb_pos(pix->mb_addr, &(pix->pos_x), &(pix->pos_y));//计算当前块左上角像素坐标
    if (luma)
    {
      pix->pos_x += pix->x;//计算出当前块周围像素点的像素坐标
      pix->pos_y += pix->y;
    }
    else
    {
