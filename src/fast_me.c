...
int PartCalMad(pel_t *ref_pic,pel_t** orig_pic,pel_t *(*get_ref_line)(int, pel_t*, int, int, int, int), int blocksize_y,int blocksize_x, int blocksize_x4,int mcost,int min_mcost,int cand_x,int cand_y)
{
  ...
  for (y=0; y<blocksize_y; y++)
    {
    ref_line  = get_ref_line (blocksize_x, ref_pic, cand_y+y, cand_x, /*img->*/height, img->width);//2004.3.3 参考图像块的某一行
    orig_line = orig_pic [y];//原始图像块的某一行
    
    for (x4=0; x4<blocksize_x4; x4++)//选中某一列 blocksize_x4=4*blocksize_x  但为什么这上面是blocksize_x4=blocksize_x>>2
    {
      mcost += byte_abs[ *orig_line++ - *ref_line++ ];//在这一列中，在前面所在行的基础上(整像素的行)，一次一次的加行，加4次(这是1/4像素之间插值的行)
    ...
    return mcost;//得到的结果是原始块和参考块在1/4像素下的SAD
}
...
FastIntegerPelBlockMotionSearch  (pel_t**   orig_pic,     // <--  not used
                  int       ref,          // <--  reference frame (0... or -1 (backward))
                  int       list,
                  int       pic_pix_x,    // <--  以图像左上角为原点 分块左上角的像素坐标absolute x-coordinate of regarded AxB block
                  int       pic_pix_y,    // <--  absolute y-coordinate of regarded AxB block
                  int       blocktype,    // <--  block type (1-16x16 ... 7-4x4)
                  int       pred_mv_x,    // <--  pred_mv_x是1/4像素mv       motion vector predictor (x) in sub-pel units
                  int       pred_mv_y,    // <--  motion vector predictor (y) in sub-pel units
                  int*      mv_x,         //  --> mv_x是整像素的mv            motion vector (x) - in pel units
                  int*      mv_y,         //  --> motion vector (y) - in pel units
                  int       search_range, // <--  1-d search range in pel units                         
                  int       min_mcost,    // <--  minimum motion cost (cost for center or huge value)
                  double    lambda)       // <--  lagrangian parameter for determining motion cost
{
  static int Diamond_x[4] = {-1, 0, 1, 0};// 顺时针 小菱形模板(-1,0) (0,1) (1,0) (0,-1)
  static int Diamond_y[4] = {0, 1, 0, -1};
  static int Hexagon_x[6] = {2, 1, -1, -2, -1, 1};//  顺时针 中六角形模板
  static int Hexagon_y[6] = {0, -2, -2, 0,  2, 2};
  static int Big_Hexagon_x[16] = {0,-2, -4,-4,-4, -4, -4, -2,  0,  2,  4,  4, 4, 4, 4, 2};//逆时针 大六角形模板
  ...
  int   pred_x        = (pic_pix_x << mvshift) + pred_mv_x;       // 图像左上角为原点 的1/4像素坐标 predicted position x (in sub-pel units)
  int   pred_y        = (pic_pix_y << mvshift) + pred_mv_y;       // predicted position y (in sub-pel units)
  int   center_x      = pic_pix_x + *mv_x;                        // 搜索中心的坐标 图像左上角为原点 的像素坐标center position x (in pel units)
  ...
  int   i,m, iSADLayer; 
  int   iAbort;
  int       N_Bframe = input->successive_Bframe;//B帧使用的数量
  ...
  {
    get_ref_line = FastLineX;//搜索框不能超出图像范围
  }
  else
  {
    get_ref_line = UMVLineX;//搜索框超出了图像范围 需要填充像素
  }
  
  //////allocate memory for search state   分配搜索状态的内存//////////////////////////
  memset(McostState[0],0,(2*search_range+1)*(2*search_range+1)*4);//1/4像素所以乘以4
  
   ///////Threshold defined for early termination 尽快停止搜索的阈值///////////////////  
  if(ref>0) 
  {
    if(pred_SAD_ref!=0)//pred_SAD_ref是利用时间相关性，取已求出的、前一帧相同位置块的MV，进行比例调节后得到的预测MV
    {
      betaSec = Bsize[blocktype]/(pred_SAD_ref*pred_SAD_ref)-AlphaSec[blocktype];//书上P305页的调整因子
  ...
  {
    if(blocktype==1)//16*16
    {
      if(pred_SAD_space !=0)//空域预测
      {
    ...
    {
      if(pred_SAD_uplayer !=0)//已求出的、同位置、上一级、大一倍块的mv预测
      {
  ...
  cand_x = center_x ;
  cand_y = center_y ;
  //pred_x        = (pic_pix_x << 2) + pred_mv_x; 
  //cand_x<<2	  =  (pic_pix_x << 2)+ *mv_x<<2; 
  //所以pred_mv_x= *mv_x<<2
  //(cand_x<<2, cand_y<<2)-(pred_x, pred_y) 1/4像素坐标 得到搜索点和预测点mv的差 然后乘以比例系数 作为MV的代价
  mcost = MV_COST (lambda_factor, mvshift, cand_x, cand_y, pred_x, pred_y);
  //当前块和参考块的SAD 这个SAD的cost不会把上面的覆盖吗?
  mcost = PartCalMad(ref_pic, orig_pic, get_ref_line,blocksize_y,blocksize_x,blocksize_x4,mcost,min_mcost,cand_x,cand_y);//
  McostState[search_range][search_range] = mcost;//保存cost 到状态
  if (mcost < min_mcost)//如果搜索到的某个点的cost更低 保存起来
  {
    min_mcost = mcost;
    best_x = cand_x;
    best_y = cand_y;
  ...
  for (m = 0; m < 4; m++)
  {   
    cand_x = iXMinNow + Diamond_x[m];//更新cand_x 以预测mv指向的地方为中心小菱形模板
    cand_y = iYMinNow + Diamond_y[m];   
    SEARCH_ONE_PIXEL     			//按照菱形模板的每一个像素检索，记录最小cost的地方
  } 

  if(center_x != pic_pix_x || center_y != pic_pix_y)
  {
    cand_x = pic_pix_x ;//更新cand_x 以分块左上角的像素坐标地方为中心小菱形模板
    cand_y = pic_pix_y ;//这个点应该是参考帧中的块 与原始的块左上角相同位置的地方
    SEARCH_ONE_PIXEL

    iXMinNow = best_x;
    iYMinNow = best_y;
    for (m = 0; m < 4; m++)//小菱形模板的各个点遍历
    {   
    ...
    if(blocktype>1)//非16*16块像素
  {
    cand_x = pic_pix_x + (pred_MV_uplayer[0]/4);//通过上层所指向mv为预测
    cand_y = pic_pix_y + (pred_MV_uplayer[1]/4);
    SEARCH_ONE_PIXEL
	//min_mcost<(1+betaThird)*pred_SAD_uplayer  遍历记录的min_mcost很小 判为很满意
    if ((min_mcost-pred_SAD_uplayer)<pred_SAD_uplayer*betaThird)
      goto third_step;//跳转到第三步
	//min_mcost>(1+betaThird)*pred_SAD_uplayer  遍历记录的min_mcost很大 判为不满意
    else if((min_mcost-pred_SAD_uplayer)<pred_SAD_uplayer*betaSec)
      goto sec_step;//跳转到第二步
  } 

  //coordinate position prediction ????
  if ((img->number > 1 + ref && ref!=-1) || (list == 1 && (Bframe_ctr%N_Bframe) > 1))  //for debug
  {
    cand_x = pic_pix_x + pred_MV_time[0]/4;//以预测mv指向的地方为中心小菱形模板
    cand_y = pic_pix_y + pred_MV_time[1]/4;//已求出的，前一帧同位置块的运动矢量
    SEARCH_ONE_PIXEL
  }
...
first_step: //Unsymmetrical-cross search 
  ...
  for(i=1;i<=search_range/2;i++)//x轴的循环更多
  {
    search_step = 2*i - 1;
    cand_x = iXMinNow + search_step;//非对称十字搜索
    cand_y = iYMinNow ;
    SEARCH_ONE_PIXEL    
  ...
  for(i=1;i<=search_range/4;i++)//y轴的循环较少
  {
    search_step = 2*i - 1;
    cand_x = iXMinNow ;
    cand_y = iYMinNow + search_step;//非对称十字搜索
    SEARCH_ONE_PIXEL
    cand_x = iXMinNow ;
  ...
  iXMinNow = best_x;
  iYMinNow = best_y;//iYMinNow是以现在搜索后代价最小的地方重新为中心
    // Uneven Multi-Hexagon-grid Search 
  for(pos=1;pos<25;pos++)
  {
    cand_x = iXMinNow + spiral_search_x[pos];
    cand_y = iYMinNow + spiral_search_y[pos];//小型螺旋全搜索
    SEARCH_ONE_PIXEL
  }
  //early termination algrithm, refer to JVT-D016
    EARLY_TERMINATION
  ...
  for(i=1;i<=search_range/4; i++)
  {
    ...
    for (m = 0; m < 16; m++)
    {
      cand_x = iXMinNow + Big_Hexagon_x[m]*i;//大六角形模板搜索
      cand_y = iYMinNow + Big_Hexagon_y[m]*i; 
      SEARCH_ONE_PIXEL1(1)//如果大六角形模板搜索里面找到了比以前更低的cost，iAbort置1
    }
    if (iAbort)
    { 
      //early termination algrithm, refer to JVT-D016
      EARLY_TERMINATION//判断结果是否满意 
    }
  }
sec_step:  //Extended Hexagon-based Search
      ...
      for(i=0;i<search_range;i++) 
      {
        ...
        for (m = 0; m < 6; m++)
        {   
          cand_x = iXMinNow + Hexagon_x[m];//中六边形模板
          cand_y = iYMinNow + Hexagon_y[m];   
          SEARCH_ONE_PIXEL1(0)
        } 
        if(iAbort)//如果中六边形模板搜索里面找到了比以前更低的cost，iAbort置0
          break;
        iXMinNow = best_x;
        iYMinNow = best_y;
      }
third_step: // the third step with a small search pattern
      ...
      for(i=0;i<search_range;i++) 
      {
        ...
        for (m = 0; m < 4; m++)
        {   
          cand_x = iXMinNow + Diamond_x[m];//小菱形模板搜索
          cand_y = iYMinNow + Diamond_y[m];   
          SEARCH_ONE_PIXEL1(0)
        } 
        if(iAbort)//如果小菱形模板搜索里面找到了比以前更低的cost，iAbort置0
          break;
        iXMinNow = best_x;
        iYMinNow = best_y;
      }

      *mv_x = best_x - pic_pix_x;//得到像素坐标的MV 运动MV搜索完成
 ...
 * 1. int AddUpSADQuarter() returns SADT of a fractiona(分数) pel MV
...
{
  static int Diamond_x[4] = {-1, 0, 1, 0};//小菱形
  static int Diamond_y[4] = {0, 1, 0, -1};
  int   mcost;
  ...
  *mv_y <<= 2;
  ...
  {
    PelY_14 = FastPelY_14;//mv在范围内
  }
  else
  {
    PelY_14 = UMVPelY_14;//mv在范围外
  }
